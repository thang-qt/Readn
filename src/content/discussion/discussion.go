...
func GetThreadAsHTML(thread *Thread) string {
	...
	sb.WriteString(`<div class="discussion-thread discussion-` + providerClass + `">`)

	// Metadata with consistent styling (similar to yarr's item metadata)
	sb.WriteString(`<div class="discussion-meta text-muted mb-3">`)
	if thread.Author != "" {
		sb.WriteString(`by <span class="discussion-author">` + thread.Author + `</span>`)
	}
	if thread.Time != "" {
		sb.WriteString(` <span class="discussion-time">` + thread.Time + `</span>`)
	}
	sb.WriteString(`</div>`)

	// Main post content (for text posts)
	if thread.Content != "" {
		sb.WriteString(`<div class="discussion-content mb-4">`)
		sb.WriteString(thread.Content)
		sb.WriteString(`</div>`)
	}
	...
	if len(thread.Comments) > 0 {
		sb.WriteString(`<hr>`)
		sb.WriteString(`<div class="discussion-comments">`)
		sb.WriteString(`<h3 class="mb-3">`)
		sb.WriteString(pluralize(CountAllComments(thread.Comments), "comment"))
		sb.WriteString(`</h3>`)
		...
		if len(thread.Comments) > 0 && len(thread.Comments[0].Children) > 0 {
			...
			sb.WriteString(renderHierarchicalComments(thread.Comments, 0))
		} else {
			// Flat structure (HN)
			sb.WriteString(renderFlatComments(thread.Comments, 0))
		}
